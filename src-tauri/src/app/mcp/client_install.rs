...
use crate::app::mcp::client_config::{
    claude_code_config_path, claude_desktop_config_path, codex_config_path,
    remove_claude_code_entries,
};
use crate::app::state::McpInstallResult;
use crate::app::support::ensure_parent_directory;

pub fn install_codex_mcp_with_port(
    server_path: &Path,
    port: u16,
) -> Result<McpInstallResult, String> {
    let config_path =
        codex_config_path().ok_or_else(|| "Codex config path could not be resolved".to_string())?;
...
pub fn install_claude_desktop_mcp_with_port(
    server_path: &Path,
    port: u16,
) -> Result<McpInstallResult, String> {
...
pub fn install_claude_code_mcp_with_port(
    server_path: &Path,
    port: u16,
) -> Result<McpInstallResult, String> {
    let config_path = claude_code_config_path()
        .ok_or_else(|| "Claude Code config path could not be resolved".to_string())?;
    ensure_parent_directory(&config_path)?;
    let mut root = fs::read_to_string(&config_path)
        .ok()
        .and_then(|raw| serde_json::from_str::<serde_json::Value>(&raw).ok())
