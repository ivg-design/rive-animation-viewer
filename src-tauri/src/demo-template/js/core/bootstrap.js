        ...
        function handleRenderSurfaceCommand(command, emitToMain) {
            ...
            if (type === 'vm-set' || type === 'vm-fire') {
                ...
                var vmAccessor = vmDescriptor.source === 'global-view-model'
                    ? resolveGlobalVmAccessor(vmDescriptor.globalViewModelName, vmDescriptor.path, vmKind)
                    : resolveLiveAccessor(vmDescriptor.path, vmKind);
                ...
                if (vmKind === 'trigger') recordRenderSurfaceTriggerReceipt(Object.assign({}, vmDescriptor, { kind: 'trigger' }));
                return {
        ...
        /* ── Canvas sizing ───────────────────────────────────── */
        function setupCanvasSize() { resizeCanvas(); }
        function setupCanvasResizeObserver() {
            if (!els.canvasContainer || typeof ResizeObserver === 'undefined') return;
            if (canvasResizeObserver) { try { canvasResizeObserver.disconnect(); } catch (e) { /* noop */ } }
            canvasResizeObserver = new ResizeObserver(function () { handleResize(); });
            canvasResizeObserver.observe(els.canvasContainer);
        }
        function resizeCanvas() {
            if (isRenderSurfaceMode && applyRenderSurfaceMediaSizing()) return true;
            ...
            scheduleCanvasViewportAlignment(container, { fixed: isFixed, width: pixelWidth, height: pixelHeight });
        }
        function buildCenteredCanvasScrollOffsets(containerWidth, containerHeight, contentWidth, contentHeight) {
            return { left: Math.max(0, Math.round((contentWidth - containerWidth) / 2)), top: Math.max(0, Math.round((contentHeight - containerHeight) / 2)) };
        }
        function scheduleCanvasViewportAlignment(container, canvasSize) {
            ...
            scheduler(function () {
                if (!canvasSize || !canvasSize.fixed) { container.scrollLeft = 0; container.scrollTop = 0; return; }
                var offsets = buildCenteredCanvasScrollOffsets(container.clientWidth, container.clientHeight, canvasSize.width, canvasSize.height);
        ...
        /* ── Canvas background ───────────────────────────────── */ function updateCanvasBackground() {
            ...
            if (themeColorMeta) themeColorMeta.setAttribute('content', normalizeCanvasColor(currentCanvasColor) || DEFAULT_CANVAS_COLOR);
        }
