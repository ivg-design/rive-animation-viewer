...
- **Global VM controls** — RAV discovers every global ViewModel used by the loaded file and displays them in a separate, initially collapsed `GLOBAL VM` section above `ROOT VM`. Each global ViewModel has its own independently collapsible tree, and same-path properties remain scoped to the correct global instance.
- **Global VM MCP tools** — Six named operations let MCP list global trees, read and set values, fire triggers, and set or clear images without falling back to the artboard-bound ViewModel.
- **Rendered-canvas capture** — `rav_capture_canvas` returns the visible authoritative playback surface as PNG image content, including its rendered background, bounded downscaling, exact PNG byte length, renderer, dimensions, and capture-surface metadata.

### Changed

- **MCP registry parity** — The bundled native sidecar and legacy JavaScript server both advertise 49 unique tools on a fresh MCP activation, including all global-VM and canvas-capture operations.
...
- **Flushed WebGL capture** — Canvas screenshots cross a refreshed-frame fence before capture, so the returned PNG reflects the control state RAV has actually rendered.
- **Rounded native overlays** — Flyout, popover, and modal backgrounds are clipped to their rounded RAV-yellow frame, eliminating dark square corners outside the outline.
- **Console palette and first-open stability** — Warning and error rows use dark RAV severity colors, and Eruda's virtualization measurement layer remains measurable so the JS console no longer flickers between painted and blank rows during initial hydration.
...
- **macOS `.riv` document declarations** — RAV declares both the official `app.rive.editor.rive-file` UTI and the pre-2.4.3 `app.rive.animation.viewer.riv` compatibility UTI as alternate Viewer types, each backed by the tracked `src-tauri/icons/RiveFileIcon.icns`. Cold and warm opens route through the native queue and `open-file` bridge. A version-and-schema-gated first launch after an update refreshes the installed bundle with `lsregister -f` without restarting Finder, changing the Viewer/Alternate rank, or taking over the default handler.
- **Windows `.riv` document icon** — Windows bundles include the dedicated ten-resolution `RiveFileIcon.ico` generated mechanically from the supplied 1024 px master. The NSIS hook replaces Tauri's executable-based `Rive File\\DefaultIcon` value on installs, repairs, and updates, then notifies Explorer. The MSI package installs the equivalent `Rive Animation Viewer.riv\\DefaultIcon` value as an upgrade-aware component that is removed on uninstall.
...
- **Runtime compatibility default** — Web 2.40.0 / runtime-v0.1.271 remains selectable, but RAV now defaults to Web 2.39.2. Live RAV MCP comparison proved that 2.40.0 can double-offset nested, data-bound images in both WebGL2 and Canvas, while 2.39.2 preserves the Rive Editor-authored positions. A one-time migration changes stored `latest` and `2.40.0` preferences to 2.39.2; a later explicit selection of Latest or 2.40.0 is retained and shows an authored-layout warning. Canary runtime-v0.1.272 remains separate.

### Fixed

- **Fixed-canvas centering and overflow** — Overflow-safe auto margins keep a fixed canvas centered while it fits, then collapse safely so an oversized canvas scrolls from its authored top-left origin. The central scroller uses the shared styled 10px scrollbar track, thumb, and corner.
- **Clean release build identity** — Distribution builds snapshot Git worktree state before creating the local build counter, so a clean CI checkout no longer labels its own signed packages `-dirty`; genuinely modified checkouts remain marked.
...
- **Live runtime evidence** — RAV MCP reproduced the Web 2.40.0 nested-image double offset in both renderers and confirmed the Rive Editor-authored layout under 2.39.2. Authored list labels remained distinct and in runtime order, every embedded raster was available to image controls, non-raster embedded resources were excluded, overflow-safe canvas centering/scrolling was exercised, and an exported standalone marker confirmed execution of the applied editor script.
