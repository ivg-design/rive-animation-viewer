...
- Current release: `2.4.3` (private acceptance candidate; not public)
- Release status: private GitHub draft pending isolated signed-updater acceptance and separate installed-app Launch Services/Finder verification. `2.4.3` is not public and must not advance the public release or `latest.json` feed until both gates pass; the updater receipt is not Finder proof.
